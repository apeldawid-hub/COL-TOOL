...
- **Złota reguła bezpieczeństwa Floor Hours**: **Floor = 32.0 h/dzień (1 Manager + 1 Barista rano/AM + 1 Manager + 1 Barista wieczorem/PM = 4 osoby x 8h = 32h/dzień)**. Dla pełnego 7-dniowego tygodnia daje to **224.0 h bazy Floor**. Grafik kawiarni nie może spaść poniżej tej bariery dobowej.
- **Pula Flex (Manipulacja Zmianami w każdym tygodniu)**: Wszystkie godziny ponad bazę Floor ($H_{\text{Tydzień}} - H_{\text{Floor}}$) tworzą pulę zmienną (Flex Labor). Silnik uczenia trendów analizuje historię popytu z poprzednich miesięcy/lat (MAPAL Fichajes) i w każdym tygodniu (W1–W5) wylicza dobowy rozkład oraz widełki manipulacji zmianami baristów (np. wzmocnienie w Sobotę i Piątek, oszczędności w dni powszednie).
...
│   ├── ImportModal.tsx              # Inteligentny import raportów z auto-detekcją (AOP P&L + MAPAL Fichajes)
...
5. **Pełny Cykl Przełomów Miesięcy (Split-Week Cross-Month Bridge)**:
   - Obsługa scalania tygodni cząstkowych z przełomu miesięcy kalendarzowych (cykl wtorek–poniedziałek, np. W5 Września 29–30.09 i W1 Października 01–05.10) w jeden spójny 7-dniowy cykl operacyjny.
   - Dwukierunkowy most w `CalculationEngine`: obsługa typu `trailing` (koniec miesiąca) oraz `leading` (początek miesiąca) z automatycznym pobieraniem zmian granicznych `boundaryShifts` (`electron/main.ts` z buforem 7 dni).
   - Scalone podsumowanie robocizny: łączne godziny menedżerów obu części ($H_{\text{MGR Part 1}} + H_{\text{MGR Part 2}}$), łączna Pula Baristów ($272\text{h} - H_{\text{MGR Łącznie}}$), 7-dniowa siatka obsady AM/PM z detekcją luk oraz interaktywny przełącznik widoku w `CrossMonthBridgeCard` i `ManagerLaborBridgeModal`.

### Algorytm 6: Flash Forecast / Pre-closing Projection dla Poniedziałku (Sposób 1)
W rytmie planowania Starbucks poniedziałek jest dniem publikacji grafiku na kolejny tydzień ($W+2$). Trwający tydzień kończy się jednak dopiero w poniedziałek wieczorem, więc pełne logowania z systemu MAPAL Fichajes spływają we wtorek rano.
1. **Detekcja warunku**:
   - `isCurrentCalendarWeek === true` oraz dzisiejszy dzień to poniedziałek (`SystemClock.now().dayOfWeek === 'Pn'`) kończący dany tydzień kalendarzowy (`w.date_to`).
2. **Kalkulacja estymacji poniedziałku (Sposób 1)**:
   - **Menedżerowie na poniedziałek ($H_{\text{MGR Today}}$)**: pobierani bezpośrednio z ułożonego grafiku Modułu 2 (`manager_schedule_shifts`) z wykluczeniem wsparć obcych i absencji.
   - **Baza celu tygodnia ($H_{\text{Target}}$)**: jeśli SM wpisał *Grafik (h)* (`scheduled_hours`), używana jest ta wartość; w przeciwnym razie plan AOP tygodnia (`w.planHours`).
   - **Godziny zalogowane z 6 dni ($H_{\text{6 dni}}$)**: rzeczywiste godziny z bazy `labor_actuals_log` (wtorek–niedziela).
   - **Barisci na poniedziałek ($H_{\text{Bar Today}}$)**: dopełnienie różnicy celu:
     $$H_{\text{Bar Today}} = \max(0, H_{\text{Target}} - H_{\text{6 dni}} - H_{\text{MGR Today}})$$
   - **Suma estymacji poniedziałku**:
     $$H_{\text{Total Monday}} = H_{\text{MGR Today}} + H_{\text{Bar Today}}$$
     $$H_{\text{Full Week Est}} = H_{\text{6 dni}} + H_{\text{Total Monday}}$$
   - **Transakcje (TRX)**: jeśli SM wprowadził transakcje z 6 dni, poniedziałek estymowany jest jako $\max(0, \text{Plan TRX} - \text{TRX}_{\text{6 dni}})$, sumując się do $\text{Plan TRX}$ (lub wyżej przy nadwyżce). W przypadku braku wpisu domyślnie przyjmowany jest pełny plan tygodnia.
3. **Wpływ na silnik MTD i HANW na $W+2$**:
   - Tydzień zostaje tymczasowo włączony do `isClosed = true` dla celów predykcji MTD i zaangażowanego budżetu robocizny, z zachowaniem statusu wizualnego `status = 'current'` oraz flagi `isMondayProjected = true`.
   - MTD Velocity i Earned Labor Budget są w 100% stabilne i odzwierciedlają pełne dwa tygodnie ($W_1 + W_2$), co eliminuje fałszywe skoki HANW na tydzień docelowy $W+2$ (`target_planning`).
4. **Interfejs Użytkownika i Bezobsługowe Zastąpienie**:
   - W tabeli harmonogramu kolumna *Act RCP (h)* prezentuje estymowaną sumę z badge'em `🔮 Pn: +Xh est.` oraz podglądem składowych `(6 dni + Pn)`.
   - Kolumna *Act TRX* prezentuje badge `🔮 Pn est.` z możliwością szybkiej edycji w dedykowanym oknie `MondayProjectionModal`.
   - Nad tabelą wyświetla się elegancki baner informacyjny Flash Forecast z przyciskiem szczegółów.
   - **We wtorek rano**: po imporcie fizycznych logowań z MAPAL Fichajes, tydzień staje się przeszły (`temporalStatus = 'past'`), a estymacja Flash Forecast automatycznie ustępuje rzeczywistym danym z bazy.
