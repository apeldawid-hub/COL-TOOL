...
export const TrainingsView: React.FC<TrainingsViewProps> = ({
  ...
  const [currentYear, setCurrentYear] = useState<number>(selectedYear || currentNow.year);
  const [currentMonth, setCurrentMonth] = useState<number>(selectedMonth || currentNow.monthIndex + 1);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* 2. Pasek Czasu i Miesiąca Szkoleń (Niezależny dla modułu) */}
      <ModuleDateBar
        year={currentYear}
        month={currentMonth}
        onPeriodChange={(y, m) => {
          setCurrentYear(y);
          const mIdx = POLISH_MONTHS.indexOf(m as any);
          if (mIdx !== -1) {
            setCurrentMonth(mIdx + 1);
          }
        }}
        extraInfo={
          <span className="text-xs text-stone-600 font-medium">
            Miesięczny budżet Zmian T (Non-Coverage): <strong className="text-emerald-800 font-bold">{TrainingEngine.calculateMonthlyNcTrainingHours(allShifts, currentYear, currentMonth).toFixed(1)}h</strong>
          </span>
        }
      />

      {/* 3. Kafelki Podsumowania i KPI */}
      <TrainingStatsCard
        partners={partners}
        allShifts={allShifts}
