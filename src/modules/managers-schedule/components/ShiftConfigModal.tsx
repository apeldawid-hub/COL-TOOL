...
export const ShiftConfigModal: React.FC<ShiftConfigModalProps> = ({
  ...
  const filteredShifts = shifts.filter(s => {
    ...
    if (activeCategory === 'flexible') return FLEXIBLE_SHIFT_CODES.has(s.code) || s.code.includes('SUP') || s.code.includes('MID');
    return true;
  });
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-stone-300 mt-0.5">
                Konfiguruj godziny bazowe, presety (MID, SUP, SAM, SPM, NC), kategorie oraz reguły niedzielne
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-3 scrollbar-thin">
          {filteredShifts.map(s => {
            ...
            return (
              <div
                key={s.code}
                className={`p-3.5 rounded-2xl border transition-all flex flex-wrap items-center gap-3 ${
                  isSunday
              ...
              >
                ...
                {/* Szybkie presety bazowe dla zmian elastycznych */}
                {(FLEXIBLE_SHIFT_CODES.has(s.code) || s.code.includes('SUP') || s.code.includes('MID')) && (
                  <div className="w-full flex items-center gap-2 pt-2.5 mt-1 border-t border-stone-200/80 text-xs flex-wrap">
                    <span className="text-[10px] uppercase font-bold text-[#006241] flex items-center gap-1">
                      <Clock className="w-3 h-3 text-[#006241]" />
                      <span>Presety bazowe:</span>
                    </span>
                    <div className="flex items-center gap-1 flex-wrap">
                      {[4.0, 5.0, 6.0, 7.0, 8.0, 9.0, 10.0].map(h => (
                        <button
                          key={h}
                          type="button"
                          onClick={() => handleUpdate(index, 'hours', h)}
                          className={`px-2 py-0.5 rounded-md text-[10px] font-bold border transition-colors cursor-pointer ${
                            Math.abs(s.hours - h) < 0.01
                    ...
                    <span className="text-[10px] text-stone-400 ml-auto italic">
                      💡 Ustawia domyślny wymiar w katalogu. Menedżer może też dostosować godziny per zmiana w grafiku.
                    </span>
                  </div>
                )}
              </div>
