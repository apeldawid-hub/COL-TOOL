...
20. **✨ AutoScheduling — Inteligentny Generator Grafiku i Solver Optymalizacyjny**:
    - Przycisk **AutoScheduling** w pasku narzędzi otwiera dedykowany pulpit generatora grafiku.
    - **Dwa inteligentne tryby**:
      - 🌟 *Smart Full*: układa cały miesiąc od zera, chroniąc wprowadzone urlopy `H`, chorobowe `L4`, dni administracyjne `NC` (w poniedziałki/wtorki), szkolenia `T` i wyjazdy `BT`.
      - 🧩 *Fill Gaps*: zachowuje wszystkie wpisane zmiany i dopełnia jedynie brakujące otwarcia, zamknięcia oraz godziny pracownikom do ich etatu.
    - **100% Zgodności z Kodeksem Pracy**: Twarda tarcza (11h odpoczynku dobowego art. 132 KP, 35h tygodniowego art. 133 KP, max 3 pracujące niedziele z rzędu art. 151^10 KP, zmiany niedzielne `AMN`/`PMN` tylko w niedziele).
    - **Obsługa Zmian MID w Szczytach**: Nadmiar godzin zespołu ponad bazę AM+PM jest automatycznie lokowany jako zmiany środkowe `MIB` (12:00–20:00) w Soboty, Piątki i Niedziele Handlowe.
    - **Maksymalizacja Zgodności z Dyspozycjami**: Silnik inteligentnie dopasowuje preferencje poranne (`M`), wieczorne (`Z`) oraz wolne (`OFF`) z matrycy dyspozycji.
    - **Interaktywny Podgląd i Diff**: Przed zatwierdzeniem Store Manager ma wgląd w kafelki KPI, bilans godzin każdego pracownika, siatkę zmian oraz zestawienie wprowadzonych modyfikacji.
    - **1-Klik Zastosuj**: Transakcyjny zapis do bazy danych z automatycznym punktem przywracania w historii wersji.
