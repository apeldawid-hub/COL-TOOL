...
/**
 * Normalizuje i formatuje oficjalne stanowisko Starbucks do zwięzłego kodu / etykiety:
 * - CUSTOMER AND SALES MANAGER -> SM (Store Manager)
 * - PEOPLE MANAGER / ASSISTANT STORE MANAGER -> ASM (Assistant Store Manager)
 * - SHIFT SUPERVISOR -> SSV
 */
export function formatManagerRole(role?: string): string {
  if (!role) return '';
  ...
  if (
    upper === 'SM' ||
    upper === 'STORE MANAGER' ||
    upper === 'CUSTOMER AND SALES' ||
    upper === 'CUSTOMER AND SALES MANAGER' ||
    upper.includes('CUSTOMER AND SALES')
  ) {
    return 'SM';
  }
  ...
  if (
    upper === 'ASM' ||
    upper === 'ASSISTANT STORE MANAGER' ||
    upper === 'PEOPLE MANAGER' ||
    upper === 'PEOPLE AND CULTURE MANAGER' ||
    upper.includes('PEOPLE MANAGER') ||
    upper.includes('ASSISTANT STORE')
  ) {
    return 'ASM';
  }

  if (upper === 'SSV' || upper === 'SHIFT SUPERVISOR' || upper.includes('SHIFT SUPERVISOR')) {
    return 'SSV';
  }

  if (upper.includes('SM IN TRAINING')) return 'SM in Tr.';
  if (upper.includes('ASM IN TRAINING')) return 'ASM in Tr.';
  if (upper.includes('SSV IN TRAINING')) return 'SSV in Tr.';

  return trimmed;
}

/**
 * Zwraca pełną czytelną nazwę stanowiska z akronimem
 */
export function formatManagerRoleFull(role?: string): string {
  const short = formatManagerRole(role);
  switch (short) {
    case 'SM':
      return 'Store Manager (SM)';
    case 'ASM':
      return 'Assistant SM (ASM)';
    case 'SSV':
      return 'Shift Supervisor (SSV)';
    case 'SM in Tr.':
      return 'SM in Training';
    case 'ASM in Tr.':
      return 'ASM in Training';
    case 'SSV in Tr.':
      return 'SSV in Training';
    default:
      return role || '';
  }
}
