          ...
          <div className="bg-stone-50 rounded-2xl p-3.5 border border-stone-200 flex items-center justify-between text-xs">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="font-black text-rose-700 bg-rose-100 px-2 py-0.5 rounded-md border border-rose-200 ml-1">
                {attemptedShiftCode} {customHours !== undefined ? `(${customHours}h)` : attemptedShiftDef?.hours ? `(${attemptedShiftDef.hours}h)` : ''}
              </span>
              {(customStartTime || attemptedShiftDef?.start_time) && (
                <span className="text-[11px] font-semibold text-stone-600 bg-stone-100 px-1.5 py-0.5 rounded border border-stone-200">
                  {customStartTime || attemptedShiftDef?.start_time} – {customEndTime || attemptedShiftDef?.end_time}
                </span>
              )}
            </div>
          </div>
