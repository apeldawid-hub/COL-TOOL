...
export class DatabaseManager {
  ...
  private seedManagerModuleData(): void {
    ...
    // Migracja ról: Normalizacja CUSTOMER AND SALES -> SM oraz ASSISTANT STORE MANAGER / PEOPLE MANAGER -> ASM
    try {
      this.db.run("UPDATE manager_employees SET role = 'SM' WHERE role = 'CUSTOMER AND SALES MANAGER' OR role LIKE '%CUSTOMER AND SALES%'");
      this.db.run("UPDATE manager_employees SET role = 'ASM' WHERE role = 'ASSISTANT STORE MANAGER' OR role = 'PEOPLE MANAGER' OR role LIKE '%PEOPLE MANAGER%'");
      this.db.run("UPDATE manager_monthly_roster SET role = 'SM' WHERE role = 'CUSTOMER AND SALES MANAGER' OR role LIKE '%CUSTOMER AND SALES%'");
      this.db.run("UPDATE manager_monthly_roster SET role = 'ASM' WHERE role = 'ASSISTANT STORE MANAGER' OR role = 'PEOPLE MANAGER' OR role LIKE '%PEOPLE MANAGER%'");
    } catch (err) {
      console.warn('Ostrzeżenie przy migracji ról:', err);
    }
