...
export const ManagerScheduleGrid: React.FC<ManagerScheduleGridProps> = ({
  ...
  // Bezpośrednie przypisanie zmiany ze skrótu klawiszowego (A, P, O, H, L, Del) z pre-flight walidacją Kodeksu Pracy
      ...
      if (e.key === 'ArrowUp') {
        ...
        if (!isReadOnly) {
          ...
          setActivePicker({
            ...
            currentDisposition: shift?.disposition || 'OFF'
          });
        }
  ...
  }, [selectedCell, data, isReadOnly, editingEventDay, violationPrompt, activePicker, handleKeyboardShiftAssign]);
    ...
    <div className="bg-white rounded-2xl shadow-xs border border-[#E2E8E5] overflow-hidden">
                ...
                >
                  ...
                  {data.daySummaries.map(d => {
                    ...
                    if (empRcp && empRcp.hours > 0) {
                      if (empRcp.unitCode && empRcp.unitCode !== '384') {
      ...
      <div className="bg-[#F7F9F8] border-t border-[#E2E8E5] px-4 py-2 flex items-center justify-between text-xs text-stone-600 select-none">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-stone-300">•</span>
          <span>
            <kbd className="px-1.5 py-0.5 rounded bg-white border border-stone-300 shadow-2xs text-[10px] font-bold">Enter / Spacja</kbd> Wszystkie zmiany
          </span>
          <span>
