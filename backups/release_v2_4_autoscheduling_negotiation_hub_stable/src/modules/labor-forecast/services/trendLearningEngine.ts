    ...
    if (monthsWithActuals.length > 0) {
      ...
      insights.push({
        ...
        description: isCurrentMonthInHighSeason
          ? `W zidentyfikowanych warunkach szczytu popytowego kawiarnia osiąga optymalną wydajność (TPLH ${avgOverPerfTplh}) przy wyuczonym buforze +${learnedOptimalBufferWeekly}h flex/tydzień ponad bazę Floor (${currentMonthClassificationDesc}). Zabezpiecza to Customer Connection w newralgicznych dniach szczytu.`
          : `W zidentyfikowanych warunkach umiarkowanego lub niskiego popytu (${currentMonthClassificationDesc}) model rekomenduje planowanie blisko bazy Floor (32h/d = 224h/tydz.) i redukcję bufora flex do ${learnedOptimalBufferWeekly}h/tydz., co chroni wskaźnik TPLH przed załamaniem.`,
        impact: `Wyuczony bufor: +${learnedOptimalBufferWeekly}h flex / tydzień`,
        confidenceScore: 91,
    ...
    const strategyOptions: {
      ...
      floor_safe: {
        ...
        description: isBaselineMode
          ? 'Brak danych sprzedażowych. Grafik planowany na minimum operacyjnym (Floor Hours 32h/d). Bezpieczny wariant do czasu pojawienia się rzeczywistych transakcji.'
          : 'Maksymalna dyscyplina kosztowa. Grafik trzymany na bazowym progu Floor Hours (32h/dzień = 224h/tydz.), sprawdzona w miesiącach spowolnienia (Styczeń–Marzec), celując w podwyższony TPLH.',
