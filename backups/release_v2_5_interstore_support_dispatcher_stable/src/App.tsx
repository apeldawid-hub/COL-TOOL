...
export const App: React.FC = () => {
  ...
  return (
    <div className="h-screen w-screen bg-[#F4F7F5] flex overflow-hidden select-none">
                ...
                extraInfo={
                  aopPlan ? (
                    <span className="text-xs text-stone-600 font-medium">
                      Plan AOP: <strong className="text-stone-800">{aopPlan.plan_sales ? `${aopPlan.plan_sales.toLocaleString('pl-PL')} zł` : '—'}</strong> | TRX: <strong className="text-stone-800">{aopPlan.plan_trx ? aopPlan.plan_trx.toLocaleString('pl-PL') : '—'}</strong> | TPLH Cel: <strong className="text-emerald-800">{aopPlan.target_tplh ? aopPlan.target_tplh.toFixed(2) : '—'}</strong>
                    </span>
                  ) : undefined
                }
              />
      ...
      {/* Centralne Okno Ustawień Konfiguracji (z dynamicznym pozycjonowaniem wg aktywnego modułu) */}
