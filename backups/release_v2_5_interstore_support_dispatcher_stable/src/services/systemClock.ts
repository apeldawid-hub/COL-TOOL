...
export class SystemClock {
  ...
  /**
   * Zwraca bieżącą datę lub ustawioną datę symulacyjną (mock)
   */
  public static getNowDate(): Date {
    return SystemClock.mockDate ? new Date(SystemClock.mockDate) : new Date();
  }

  /**
   * Ustawia czas symulacyjny na potrzeby testów lub scenariuszy planowania
   */
  public static setMockDate(date: Date | string | null): void {
    if (date === null) {
      SystemClock.mockDate = null;
    } else {
      SystemClock.mockDate = typeof date === 'string' ? new Date(date) : new Date(date);
    }
    SystemClock.cachedState = SystemClock.createTimeState(SystemClock.getNowDate());
    SystemClock.listeners.forEach((listener) => listener(SystemClock.cachedState));
  }

  /**
   * Resetuje czas symulacyjny do rzeczywistego czasu systemowego
   */
  public static resetMockDate(): void {
    SystemClock.setMockDate(null);
  }

  /**
   * Tworzy zunifikowaną strukturę stanu czasu systemowego
   */
  public static createTimeState(date: Date = SystemClock.getNowDate()): SystemTimeState {
  ...
  public static now(): SystemTimeState {
    return SystemClock.createTimeState(SystemClock.getNowDate());
  }
  ...
  public static isCurrentWeek(dateFromStr: string, dateToStr: string, year: number): boolean {
    ...
    try {
      const now = SystemClock.getNowDate();
   ...
   */
  public static getNextPlanningMonday(fromDate: Date = SystemClock.getNowDate()): {
    mondayDate: Date;
