...
1. **Rejestracja RCP z systemu MAPAL Fichajes (`labor_actuals_log`)**:
   - Kolumna RCP dla menedżerów odzwierciedla zarejestrowany czas pracy z systemu MAPAL.
   - W przypadku menedżerów logowania obejmują wszystkie zmiany — **również te zrealizowane w innych kawiarniach** (wsparcia SAM, SPM, SUP).
   - Parser importu MAPAL (`mapalParser.ts`) dopasowuje nazwiska menedżerów (`NAZWISKO, IMIĘ` ➔ `manager_employees.name`) i zapisuje do bazy logowania menedżerów ze wszystkich lokali, nie zaburzając kalkulacji robocizny kawiarni Janki w Module 1.
...
## 🗄️ 5. Tabele SQLite Obsługujące Moduł 2
- `manager_employees`: lista kierowników (id, name, role, contract_type, contract_hours_ratio, hourly_rate, sort_order, is_active).
...
- Indeksy: `idx_mgr_shifts_ym`, `idx_mgr_shifts_emp`, `idx_mgr_events_ym`, `idx_mgr_versions_ym`.
