...
function setupIpcHandlers() {
  ...
  ipcMain.handle('db:get-manager-schedule-data', (_event, year: number, month: number) => {
    ...
    // Pierwsze 3 dni z kolejnego miesiąca (dni 1..3)
    const nextShiftsStmt = db.prepare(`
      SELECT id, year, month, day, date, employee_id, shift_code, hours, notes, disposition
      FROM manager_schedule_shifts
      WHERE year = ? AND month = ? AND day <= 3
      ORDER BY day ASC
    `);
  ...
  // Aktualizacja zespołu menedżerów
  ipcMain.handle('db:manage-employees', (_event, employees: any[]) => {
    const db = dbManager.getDb();
    for (const emp of employees) {
      if (emp.id) {
        db.run(`
          UPDATE manager_employees SET
            name = ?,
            role = ?,
            contract_type = ?,
            contract_hours_ratio = ?,
            hourly_rate = ?,
            sort_order = ?,
            is_active = ?
          WHERE id = ?
