...
export const ManagerScheduleView: React.FC<ManagerScheduleViewProps> = ({
  ...
  // Zapis zmian w składzie zespołu menedżerskiego dla wybranego miesiąca
  const handleSaveTeam = async (payload: { employees: ManagerEmployee[]; propagateToFuture: boolean }) => {
    if ((window as any).api?.manageEmployees) {
      await (window as any).api.manageEmployees({
        year: selectedYear,
        month: selectedMonth,
        employees: payload.employees,
        propagateToFuture: payload.propagateToFuture
      });
      await loadScheduleData();
    }
  };

  // Pobranie i odtworzenie składu z poprzedniego miesiąca
  const handleCopyPreviousMonthRoster = async () => {
    if ((window as any).api?.copyRosterFromPreviousMonth) {
      await (window as any).api.copyRosterFromPreviousMonth(selectedYear, selectedMonth);
      await loadScheduleData();
    }
  };
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      {showTeamModal && scheduleData && (
        <ManagerTeamModal
          ...
          onCopyPreviousMonth={handleCopyPreviousMonthRoster}
