  ...
  public static validateLaborLaw(
    ...
    for (let day = 1; day < totalDays; day++) {
      ...
      if (isS1Working && isS2Working) {
        ...
        const endHour1 = parseTimeToHours(s1.custom_end_time || def1?.end_time);
        const startHour2 = parseTimeToHours(s2.custom_start_time || def2?.start_time);
        ...
        if (endHour1 !== null && startHour2 !== null) {
          ...
          if (restHours < 11.0) {
            addViolation({
              ...
              message: `Przerwa między zmianą ${s1.shift_code} (koniec ${s1.custom_end_time || def1?.end_time || '22:30'}) w dniu ${day} a zmianą ${s2.shift_code} (początek ${s2.custom_start_time || def2?.start_time || '07:00'}) w dniu ${day + 1} wynosi tylko ${restHours}h.`,
    ...
    if (boundaryShifts?.prevMonthShifts && boundaryShifts.prevMonthShifts.length > 0) {
      ...
      if (prevLast && s1) {
        ...
        if (isPrevWorking && isS1Working) {
          ...
          const def1 = shiftDefMap.get(s1.shift_code);
          const endHourPrev = parseTimeToHours(prevLast.custom_end_time || defPrev?.end_time);
          const startHour1 = parseTimeToHours(s1.custom_start_time || def1?.start_time);
          ...
          if (endHourPrev !== null && startHour1 !== null) {
            ...
            if (restHours < 11.0) {
              addViolation({
                ...
                message: `Przerwa między zmianą ${prevLast.shift_code} z ostatniego dnia poprzedniego miesiąca (${prevMonthLastDay}.${String(prevMonth).padStart(2, '0')}, koniec ${prevLast.custom_end_time || defPrev?.end_time || '22:30'}) a zmianą ${s1.shift_code} w dniu 1 (początek ${s1.custom_start_time || def1?.start_time || '07:00'}) wynosi tylko ${restHours}h.`,
    ...
    if (boundaryShifts?.nextMonthShifts && boundaryShifts.nextMonthShifts.length > 0) {
      ...
      if (nextFirst && sLast) {
        ...
        if (isLastWorking && isNextWorking) {
          ...
          const endHourLast = parseTimeToHours(sLast.custom_end_time || defLast?.end_time);
          const startHourNext = parseTimeToHours(nextFirst.custom_start_time || defNext?.start_time);
          ...
          if (endHourLast !== null && startHourNext !== null) {
            ...
            if (restHours < 11.0) {
              addViolation({
                ...
                message: `Przerwa między zmianą ${sLast.shift_code} w dniu ${totalDays} (koniec ${sLast.custom_end_time || defLast?.end_time || '22:30'}) a zaplanowaną zmianą ${nextFirst.shift_code} w dniu 1.${String(nextMonth).padStart(2, '0')} (początek ${nextFirst.custom_start_time || defNext?.start_time || '07:00'}) wynosi tylko ${restHours}h.`,
