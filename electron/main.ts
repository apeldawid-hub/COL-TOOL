...
async function createWindow() {
  ...
  // Rejestracja okna głównego w AppUpdater
  AppUpdater.getInstance().setMainWindow(mainWindow);
...
function setupIpcHandlers() {
  ...
  ipcMain.handle('db:get-actual-hours-for-month', (_event, year: number, month: string) => {
    ...
    const stmt = db.prepare(`
      ...
      WHERE week_key LIKE ?
        AND (unit_code = '18120' OR unit_code = '384' OR unit_name LIKE '%108120%' OR unit_name LIKE '%18120%' OR unit_code IS NULL OR unit_code = '')
      GROUP BY week_key
    `);
    stmt.bind([pattern]);
  ...
  ipcMain.handle('db:save-floor-rules', (_event, rules: any[]) => {
    ...
    db.run(`UPDATE stores SET weekly_floor_hours = ? WHERE store_code = '18120' OR store_code = '384'`, [weeklySum]);
  ...
  ipcMain.handle('db:save-manager-shift', (_event, shift: any) => {
    ...
    const stmt = db.prepare(`
      ...
      ON CONFLICT(year, month, day, employee_id) DO UPDATE SET
        ...
        notes = excluded.notes,
        disposition = COALESCE(excluded.disposition, manager_schedule_shifts.disposition, ''),
  ...
  // Zbiorczy zapis całego grafiku menedżerskiego (AutoScheduling / Masowa aktualizacja)
  ipcMain.handle('db:save-batch-manager-shifts', (_event, payload: {
    year: number;
    month: number;
    shifts: Array<{
      day: number;
    ...
    // Automatyczny punkt przywracania w wersjach grafiku
    try {
      const numRes = db.exec(`SELECT MAX(version_num) FROM manager_schedule_versions WHERE year = ${year} AND month = ${month}`);
      const maxNum = (numRes.length > 0 && numRes[0].values.length > 0 && numRes[0].values[0][0]) ? Number(numRes[0].values[0][0]) : 0;
      ...
      vStmt.run([
        ...
        month,
        maxNum + 1,
        `✨ AutoScheduling: Wygenerowano grafik`,
        `Automatyczne wygenerowanie i zbalansowanie grafiku miesięcznego dla ${shifts.length} komórek.`
      ]);
      vStmt.free();
    } catch (verErr) {
      console.warn('Nie udało się utworzyć wersji przed zapisem AutoScheduling:', verErr);
    }
    ...
    try {
      ...
      try { db.run('ROLLBACK;'); } catch (_) {}
      console.error('Błąd podczas zbiorczego zapisu grafiku AutoScheduling:', err);
      throw err;
    }
  });
  ...
  // Inteligentny Import Raportów (2-etapowa weryfikacja z edycją przed zatwierdzeniem)
  ipcMain.handle('import:parse-report-file', async (_event, filePath: string) => {
    return await UniversalReportParser.parseReportForPreview(filePath);
  });

  ipcMain.handle('import:parse-report-buffer', async (_event, arrayBuffer: ArrayBuffer) => {
    const buffer = Buffer.from(arrayBuffer);
    return await UniversalReportParser.parseReportForPreview(buffer);
  });

  ipcMain.handle('import:commit-aop', async (_event, payload: { year: number; months: any[] }) => {
    const result = await UniversalReportParser.commitAopData(payload.year, payload.months);
  ...
  ipcMain.handle('import:commit-mapal', async (_event, payload: { records: any[] }) => {
    const result = await UniversalReportParser.commitMapalData(payload.records);
    if (result.success && mainWindow) {
      mainWindow.webContents.send('data:refreshed');
    }
    return result;
  ...
  const handleUniversalImportBuffer = async (arrayBuffer: ArrayBuffer) => {
    const buffer = Buffer.from(arrayBuffer);
    const result = await UniversalReportParser.parseAndImport(buffer);
    if (result.success && mainWindow) {
      mainWindow.webContents.send('data:refreshed');
    }
    return result;
  ...
  ipcMain.handle('import:report-file', (_event, filePath: string) => handleUniversalImportFile(filePath));
  ipcMain.handle('import:report-buffer', (_event, arrayBuffer: ArrayBuffer) => handleUniversalImportBuffer(arrayBuffer));
  ipcMain.handle('import:fichajes-file', (_event, filePath: string) => handleUniversalImportFile(filePath));
  ipcMain.handle('import:fichajes-buffer', (_event, arrayBuffer: ArrayBuffer) => handleUniversalImportBuffer(arrayBuffer));
  ...
  ipcMain.handle('db:export-historical-package', (_event, customTargetDir?: string) => {
    return dbManager.exportHistoricalDataPackage(customTargetDir);
  });

  ipcMain.handle('db:import-historical-package', (_event, customSourceDir?: string) => {
    const res = dbManager.importHistoricalDataPackage(customSourceDir);
    if (res.success && mainWindow) {
      mainWindow.webContents.send('data:refreshed');
    }
    return res;
  ...
  ipcMain.handle('db:reset-to-clean-slate', () => {
    const res = dbManager.resetToCleanSlate();
    if (res.success && mainWindow) {
      mainWindow.webContents.send('data:refreshed');
    }
    return res;
  ...
  // ==========================================
  ipcMain.handle('db:get-training-partners', () => {
    dbManager.reloadFromDisk();
    return dbManager.getTrainingPartners();
  });

  ipcMain.handle('db:save-training-partner', (_event, partner: any) => {
    const id = dbManager.saveTrainingPartner(partner);
    if (mainWindow) {
      mainWindow.webContents.send('data:refreshed');
    }
    return id;
  ...
  ipcMain.handle('db:delete-training-partner', (_event, id: number) => {
    const res = dbManager.deleteTrainingPartner(id);
    if (mainWindow) {
      mainWindow.webContents.send('data:refreshed');
    }
    return res;
  ...
  ipcMain.handle('db:get-training-shifts', (_event, partnerId?: number) => {
    dbManager.reloadFromDisk();
    return dbManager.getTrainingShifts(partnerId);
  });

  ipcMain.handle('db:save-training-shifts-batch', (_event, partnerId: number, shifts: any[]) => {
    const res = dbManager.saveTrainingShiftsBatch(partnerId, shifts);
    if (mainWindow) {
      mainWindow.webContents.send('data:refreshed');
    }
    return res;
  ...
  ipcMain.handle('db:update-training-shift', (_event, shift: any) => {
    const res = dbManager.updateTrainingShift(shift);
    if (mainWindow) {
      mainWindow.webContents.send('data:refreshed');
    }
    return res;
  ...
  ipcMain.handle('db:get-training-skill-checks', (_event, partnerId?: number) => {
    dbManager.reloadFromDisk();
    return dbManager.getTrainingSkillChecks(partnerId);
  });

  ipcMain.handle('db:save-training-skill-check', (_event, check: any) => {
    const id = dbManager.saveTrainingSkillCheck(check);
    if (mainWindow) {
      mainWindow.webContents.send('data:refreshed');
    }
    return id;
